   ...
   if (pairingCode && !XeonBotInc.authState.creds.registered) {
      ...
      if (!!phoneNumber) {
         ...
         if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
            console.log(chalk.bgBlack(chalk.redBright("Start with country code of your WhatsApp Number, Example : +254757959246")))
            process.exit(0)
         }
      } else {
         phoneNumber = await question(chalk.bgBlack(chalk.greenBright(`Please type your WhatsApp number 😘\nFor example: +254757959246 : `)))
         phoneNumber = phoneNumber.replace(/[^0-9]/g, '')
...
│ Heyyy 🥳
└┬❖  
┌┤✑  Thanks for choosing the first version of uchiha md
│└────────────┈ ⳹        
│©2025-2099 reincarnateduchiha
└─────────────────┈ ⳹ ⳹\n\n ` }, {quoted: xeonses});
              await delay(1000 * 2) 
              process.exit(0)
        }
